...
################################################################################
  ...
  for (r in 1:rnum){
    ...
    for (d in 1:dnum){
      plot(unlist(dtax[,,d]), type="l", col =paste(color[d]), lwd=2, main=paste(ratio[r])
    ...
    axis(1, 1:length(colnames(income_dta))
         , format(as.Date(colnames(income_dta), format="%Y"), "%Y"))  # X
  }
}